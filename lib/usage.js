...
class UsageTracker {
  ...
  constructor({ config, logger } = {}) {
    ...
    /** @type {Map<string, {cost:number, updatedAt:number}>} */
    this._costs = new Map();
  }
  ...
  /**
   * Cumulative USD reported for one Orchestra session, fed by the hook bus.
   * Hooks report a running total, so the value only ever moves up: a lower
   * report is a truncated or replayed payload, not a refund.
   *
   * @returns {number|null} the stored total, or null when the input was unusable
   */
  recordSessionCost(sessionId, cost) {
    if (typeof sessionId !== 'string' || !sessionId) return null;
    const n = typeof cost === 'number' ? cost : Number(cost);
    if (!Number.isFinite(n) || n < 0) {
      this.log.debug(`usage: ignoring non numeric cost for session ${sessionId}`);
      return null;
    }
    const previous = this._costs.get(sessionId);
    const total = previous ? Math.max(previous.cost, n) : n;
    this._costs.set(sessionId, { cost: total, updatedAt: Date.now() });
    return total;
  }

  /**
   * @returns {{total:number, bySession:Object<string,{cost:number,updatedAt:number}>}}
   */
  costs() {
    const bySession = {};
    let total = 0;
    for (const [id, entry] of this._costs) {
      bySession[id] = { cost: entry.cost, updatedAt: entry.updatedAt };
      total += entry.cost;
    }
  ...
  costFor(sessionId) {
    const entry = this._costs.get(sessionId);
    return entry ? entry.cost : 0;
  }

  /** Drop a closed session's cost so the map does not grow without bound. */
  forgetSession(sessionId) {
    return this._costs.delete(sessionId);
  }
}
