...
class ApprovalQueue extends EventEmitter {
   ...
   */
  constructor({ sessions = null, policy = null, config = defaultConfig, logger = NOOP_LOGGER } = {}) {
    super();
    this.sessions = sessions;
    /** Repository-committed policy, consulted ahead of any stored rule. */
    this.policy = policy;
    this.config = config;
  ...
  request({ sessionId, tool, input, cwd } = {}) {
    ...
    // The committed policy speaks first, and its word is final. A stored
    // "always allow" is one operator's past click on one machine; a policy is
    // a reviewed file in the repository the tool call is about to touch. If the
    // two disagree, the reviewed one has to win or committing it means nothing.
    const governed = this.policy
      ? this.policy.evaluate({ tool: toolName, matchText: entry.matchText, cwd: effectiveCwd })
      : null;

    if (governed && governed.decision !== 'ask') {
      const result = {
        id,
        ...
        source: 'policy',
        policyFile: governed.file,
      };
      entry.policy = { file: governed.file, rule: governed.rule };
      this.audit(entry, result);
      this.emit('resolved', this.toWireResolution(entry, result));
      return Promise.resolve(result);
    }

    // A policy that says "ask" is asking for a human, so the stored rules are
    // deliberately not consulted: the shortcut is the thing it is overriding.
    const matched = governed ? null : this.matchRule(entry);
    if (matched) {
      matched.hits += 1;
