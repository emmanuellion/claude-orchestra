...
function projectLabel(cwd) {
  ...
  // A Windows path must not go through path.resolve on POSIX: "C:/" is not
  // absolute there, so it gets resolved against the server's own directory and
  // the drive letter ends up buried in the middle of the result instead of at
  // the front where the check below looks for it.
  const isWindowsPath = /^[a-zA-Z]:[\\/]/.test(cwd);
  const full = isWindowsPath ? cwd : path.resolve(cwd);
