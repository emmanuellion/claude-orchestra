...
const S2C = {
  ...
  /** Spend ledger and caps, whenever either moves. */
  BUDGET: 'budget',
  RACE: 'race',
  ERROR: 'error',
  PONG: 'pong',
...
const KIND = {
  ...
  /**
   * An agent Orchestra did not spawn, on this machine or another one, known
   * only through the hook events it posts back. It has full agent state and
   * takes part in approvals, budgets and the digest; it has no PTY, so it has
   * no terminal and accepts no input.
   */
  EXTERNAL: 'external',
};
...
const LIMITS = {
  MAX_SESSIONS: 24,
  /** External agents tracked at once. They cost a record, not a process. */
  MAX_EXTERNAL: 32,
  /** An external agent silent this long is presumed gone. */
  EXTERNAL_TTL_MS: 30 * 60 * 1000,
