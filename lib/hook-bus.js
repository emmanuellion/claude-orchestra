...
const { STATUS, HOOK_EVENT, KIND } = require('./protocol');
...
class HookBus extends EventEmitter {
  ...
  constructor({ sessions, config, logger } = {}) {
    ...
    /** Whether an unmatched event may register an agent we did not spawn. */
    this.adoptExternal = this.config.adoptExternal !== false;
  ...
  ingest(eventName, payload) {
    ...
    let { session, matched, reason } = this._resolve(body);

    // An agent we did not spawn. Its hooks already reach us and its permission
    // requests already queue here; adopting gives that agent somewhere to be
    // seen, on this machine or any other one pointed at this server.
    if (!session && this.adoptExternal) {
      const adopted = this._adopt(body, reason);
      if (adopted) {
        session = adopted;
        matched = true;
        reason = null;
  ...
  /**
   * Decides whether an unmatched event should become an external session.
   *
   * Only two failure reasons qualify. `ambiguous-cwd` deliberately does not:
   * two local sessions sharing a directory is the case where we already refuse
   * to guess, and adopting there would hang a phantom agent next to two real
   * ones. Without a Claude session id there is nothing stable to key on, so
   * every event would create another record.
   */
  _adopt(body, reason) {
    if (reason !== 'unknown-session-id' && reason !== 'no-cwd-match') return null;
    const claudeSessionId = typeof body.session_id === 'string' ? body.session_id : '';
    if (!claudeSessionId) return null;
    if (typeof this.sessions.adoptExternal !== 'function') return null;

    const session = this.sessions.adoptExternal({
      claudeSessionId,
      host: typeof body.orchestraHost === 'string' ? body.orchestraHost : null,
      cwd: typeof body.cwd === 'string' ? body.cwd : null,
    });
    if (session) {
      this._log('info', `adopted external agent ${session.name} (${session.id})`);
    }
    return session;
  }
  ...
  _apply(session, name, body, event, ts) {
    ...
    switch (name) {
      case HOOK_EVENT.SESSION_START: {
        ...
        // git runs here, but an external agent's cwd is a path on its own
        // machine. Resolving it locally would report this repo's branch, or a
        // stranger's, as that agent's.
        if (session.kind !== KIND.EXTERNAL) this._refreshGitBranch(session);
        return;
      }
