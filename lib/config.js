...
const config = {
  ...
  autoResumeFile: path.join(ORCHESTRA_DIR, 'auto-resume.json'),
  budgetFile: path.join(ORCHESTRA_DIR, 'budget.json'),
  pushFile: path.join(ORCHESTRA_DIR, 'push.json'),
  ...
  /**
   * Typing a prompt back into a session whose quota block has expired. Off by
   * default and it stays that way: everything below only decides *how* it
   * behaves once a human has switched it on in Settings, which is also where
   * the values are edited. These are the defaults that panel starts from.
   */
  autoResume: envFlag('ORCHESTRA_AUTO_RESUME', false),
  autoResumeText: process.env.ORCHESTRA_AUTO_RESUME_TEXT || 'continue',
  /** Seconds waited past the reset instant. Resets are not clock exact. */
  autoResumeGraceSeconds: envInt('ORCHESTRA_AUTO_RESUME_GRACE', 60),
  /** Seconds between two sessions resuming, so they do not re-block together. */
  autoResumeStaggerSeconds: envInt('ORCHESTRA_AUTO_RESUME_STAGGER', 30),
  /** Resumes tried on one session before it is left alone. */
  autoResumeMaxAttempts: envInt('ORCHESTRA_AUTO_RESUME_MAX', 3),
  /** Seconds a due resume waits for the session to be safe to type into. */
  autoResumeWaitSeconds: envInt('ORCHESTRA_AUTO_RESUME_WAIT', 600),

  /**
   * Spend caps. Orchestra has always measured cost; these are what make it act
   * on the number. A cap of 0 is no cap. `lock` freezes the session at the
   * limit and keeps its context and scrollback; `warn` only raises an alert.
   */
  budgetEnabled: envFlag('ORCHESTRA_BUDGET', false),
  budgetSessionCap: Number(process.env.ORCHESTRA_BUDGET_SESSION) || 0,
  budgetDailyCap: Number(process.env.ORCHESTRA_BUDGET_DAILY) || 0,
  budgetAction: process.env.ORCHESTRA_BUDGET_ACTION === 'warn' ? 'warn' : 'lock',

  /**
   * The `sub` claim in every VAPID token, which is how a push service reaches
   * the operator of a misbehaving server. A mailto: or https: URL.
   */
  pushSubject: process.env.ORCHESTRA_PUSH_SUBJECT || 'mailto:orchestra@localhost',

  /**
   * Whether a hook event that matches no local session may register the agent
   * that sent it. This grants no new access: those hooks already hold a valid
   * token and their approvals already queue here. It only decides whether the
   * agent is visible or invisible while that happens.
   */
  adoptExternal: envFlag('ORCHESTRA_ADOPT_EXTERNAL', true),

  /**
   * Whether an `allow` rule in a repository's .orchestra-policy.json may skip
   * the human. Off by default: that file arrives with `git clone`, so obeying
   * its allows would let a repository grant permissions to an agent working on
   * it. Deny and ask from the same file always apply.
   */
  trustRepoPolicy: envFlag('ORCHESTRA_TRUST_REPO_POLICY', false),
